...
/**
 * Per-type differences between the createXFileInParent flows; everything else
 * (validation, plugin resolution, structure node, in-memory edited file) is
 * shared by createFileInParent.
 */
interface CreateFileSpec {
  /** Label used in the console error message, e.g. "markdown file". */
  label: string;
  /** Tree-node sufix and createEditedFile extension. */
  extension: string;
  /** Full on-disk filename built from the user-entered stem. */
  fileName: (name: string) => string;
  /** Display name for the tree node / editor tab; defaults to the stem. */
  displayName?: (name: string) => string;
  /** Seed content; receives the owning plugin (if any) for plugin-aware seeds. */
  initialContent: (name: string, plugin: Plugin | null | undefined) => string;
}

const createFileInParent = async (
  name: string,
  parentFolderID: string,
  spec: CreateFileSpec
) => {
  try {
    ...
    const displayName = spec.displayName ? spec.displayName(name) : name;
    const initialContent = spec.initialContent(name, plugin);
    ...
    console.error(`Failed to create ${spec.label}:`, error);
    addErrorMessage((error as Error).message, "error");
  }
};
