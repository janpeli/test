...
export class JSONSchemaToZod {
  ...
  private static parseArray(schema: JSONSchema): ZodTypeAny {
    ...
    const itemSchema = Array.isArray(schema.items)
      ? z.union(
          schema.items.map((item) => this.parseSchema(item)) as [
            ZodTypeAny,
            ZodTypeAny,
            ...ZodTypeAny[]
          ]
  ...
  private static parseObject(schema: JSONSchema): ZodObject<any> {
    ...
    if (schema.additionalProperties === true) {
      ...
      zodObject = z
        .object(shape)
        .catchall(this.parseSchema(schema.additionalProperties))
        .strip();
    } else {
  ...
  private static parseCombinator(schema: JSONSchema): ZodTypeAny {
    if (schema.oneOf) {
      if (schema.oneOf.length === 1) {
        ...
        return z.union(
          schema.oneOf.map((subSchema) => this.parseSchema(subSchema)) as [
            ZodTypeAny,
            ZodTypeAny,
            ...ZodTypeAny[]
          ]
    ...
    if (schema.anyOf) {
      if (schema.anyOf.length === 1) {
        ...
        return z.union(
          schema.anyOf.map((subSchema) => this.parseSchema(subSchema)) as [
            ZodTypeAny,
            ZodTypeAny,
            ...ZodTypeAny[]
          ]
